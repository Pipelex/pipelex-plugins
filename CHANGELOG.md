...
- **`pipelex-inputs` uploads local files so they can actually be run.** The skill gained a prepare step between assembling `inputs.json` and offering the run: it calls the new **`mthds_prepare_inputs`** tool with the same target (and the same `pipe_ref`, when Step 2 passed one) plus the filled inputs, which uploads every file-bearing value to Pipelex storage, rewrites it to a `pipelex-storage://` reference, and gets written back over `inputs.json`. The tool joins the skill's Claude `allowed-tools` and its Requirements section as a **required** dependency whenever a file-ish value isn't already `http(s)` or `pipelex-storage://` — with the stop-on-`config` discipline the other tools already carry, and no produced-invalid arm to branch on (an unresolvable closure is a no-verdict `input_domain` error, recovered through `mthds_validate` / `mthds_inputs_template`). Copies in `<output_dir>/inputs/` stay on disk for the user's own reference; only the values in `inputs.json` change, and file values change shape too — a bare path or URL becomes the canonical `{"url": "…"}` content dict. Local paths are expanded to absolute before the call: `inputs.json` stores them relative to itself, but the MCP server resolves a local path against its own working directory, so a relative value fails to read — `inputs.json` keeps its relative paths, only the submitted values are resolved.

- **`pipelex-inputs` targets registered catalog methods by id.** The MCP tools (`mthds_validate`, `mthds_inputs_template`, `mthds_run`) now accept a registered method's catalog id (`mt_…`) as `method_id` in place of submitted files, and the skill uses it: prepare inputs for — and offer to run — a catalog method with no local bundle. A by-id call operates on the method's current stored content and requires an API key (the catalog is org-scoped). Since methods aren't versioned, a by-id run re-checks the template for drift immediately before running and refuses to spend credit against a signature that changed since inputs were prepared. The file-based skills stay submitted-files only, per the new decision entry in `docs/decisions.md`.
...
- **Breaking: every `mthds_inputs_template` call pins `explicit: false`.** `pipelex-mcp` flipped the tool's `explicit` default to `true` (the ceremonial `{concept, content}` envelope per input); the skills stay on the light shape — bare example values — which their fill strategies, value-shape tables, and examples are written around, so `pipelex-inputs`, `pipelex-design`, and `pipelex-edit` now pass the flag explicitly. Nothing downstream depends on the choice: `mthds_prepare_inputs` and `mthds_run` accept both shapes identically. Rationale in `docs/decisions.md`.
