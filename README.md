...
The plugin also declares the **`pipelex-mcp` server**, which the MCP-backed skills (`pipelex-design`, `pipelex-organize`, `pipelex-inputs`) use for validation and input templates; it connects automatically. The baked URL is a placeholder until the server is deployed — point the session at a running server with:
...
**MCP server (automatic):** the plugin manifest declares the `pipelex-mcp` server (streamable HTTP), so Codex connects it on its own — the tools reach the model as `mcp__pipelex__mthds_validate` / `mcp__pipelex__mthds_inputs_template`, and `codex mcp list` shows the `pipelex` entry. The baked URL is a placeholder until the server deploys; Codex does no env expansion in MCP config, so point sessions at a running server with a same-named override, which outranks the plugin declaration:
