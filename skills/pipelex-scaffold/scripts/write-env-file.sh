...
[ $# -eq 1 ] || refuse usage
dir=$(cd "$1" 2> /dev/null && pwd) || refuse no-directory
...
origin=file
if grep -q '^PIPELEX_API_KEY=.' "$env"; then
  key=kept
...
# The plane the file resolves to: its last base-URL line, as a dotenv reader takes it.
url=$(sed -n 's/^PIPELEX_BASE_URL=//p' "$env" | tail -n 1)
url=${url%\"}
url=${url#\"}
url=${url%/}
