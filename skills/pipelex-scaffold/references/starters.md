...
**The env file's two Pipelex lines are one credential.** Both examples ship `PIPELEX_BASE_URL=https://api.pipelex.com` beside an empty `PIPELEX_API_KEY`, and a key is refused by every plane but the one that issued it. So when `/pipelex-scaffold` fills the key from the shell environment it fills the base URL from there too, whenever the shell sets one, in the same command; a base URL the shell sets with no key beside it is not copied, and an env file that already carries a key keeps both lines as they are. The command, its one guard and the report's wording for the plane the file points at are in the skill's env-file step.
