...
**A pull has no link to judge.** When the error came from `mthds_get_method`, nothing was written and nothing on disk is touched. An empty stored source answers at the same location and is not a miss: relay it as the pull's report step says. Otherwise relay the answer and its hint, and when the hint says the id is not visible, say that a method saved from another organization, or on another plane, reads exactly like a miss, and offer to find the method by name, as the skill's "List and find" says.
