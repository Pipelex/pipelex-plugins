...
7. **The local copy** — when `.local-mcp/pipelex/hooks/launch-pipelex-mcp.sh` exists, its `exec` line says which workshop the last `make claude-local-mcp` in this checkout rendered. It is ignored by git and never shipped, so it is information, not dev state to clean up.
