...
1. **`make check-hook-fresh`, before the version bump** — the vendored hook bundle is not behind its sources: its `@pipelex/tools-wasm` is npm's latest, and a rebuild in `../pipelex-sdk-js`, that repo's main checkout, is identical to it below the three-line banner. It needs that checkout on `dev`, clean and level with `origin/dev` (it fetches, and names the fast-forward when the checkout is behind), and the network; `npm ci` there rewrites its ignored `node_modules`. CI cannot ask either question, having no sibling checkout, so this gate is the only thing that does. A `STALE` line stops the release: the re-vendor is a change of its own and travels to `dev` like any other, on a branch with its changelog entry (`make vendor-hook`, then `make build`), preceded by the pin bump in `pipelex-sdk-js` when the gate says that repo still pins an older engine, and the release starts over from the `dev` that carries it. A `REFUSED` line is not a pass either: fix what it names and run the gate again. It runs first so that a stale bundle stops a release that has changed nothing yet. `docs/hooks.md`, "Re-vendoring check.mjs", says what each question compares.
2. **`make agent-check`** — `fix-unused-imports`, `ruff format`, `ruff check --fix`, and then `make check`. **It rewrites files**, so whatever it touched joins the release commit. `make check` is the substantive part: `check-shared` re-renders every target with `scripts/gen_skill_docs.py --target all --check` and reports each generated file that is `MISSING`, an `ORPHAN`, or byte-different from what the templates produce — generated `SKILL.md` files, the per-skill `references/` copies, and the `.agents/plugins/marketplace.json` discovery copy alike — then runs `ruff format --check`, `ruff check`, a second `ruff check --select=F401 --no-fix` that reports the unused imports the lint config otherwise ignores, `pyright` and `mypy`; `check-claude` and `check-codex` check the two marketplaces against the target configs. A freshness failure means someone edited a template or a shared reference without rebuilding, and the cure is `make build`, whose output then rides in the release commit.
3. **`make agent-test`** — the pytest suite, quiet unless it fails. `pyproject.toml`'s `addopts = "-m 'not recipes'"` deselects the suite that executes the shipped synthetic-inputs recipes, which is why that one is not a release gate: `make test-recipes` runs it, and it downloads packages on a cold `uv` cache.
4. **After the bump — `make build`, then `make agent-check` again.** Until the build has run, the generated manifests still carry the old number and `check_target_plugin_versions` fails with `plugin.json has A.B.C, targets/<name>.toml has X.Y.Z`. `Checks` runs the same `make check` on the pull request (see **CI on the release pull request**), so a half-stamped tree goes red there too — but no ruleset requires that check, so this pass is still what actually stops one reaching `main`.
...
The play opens no pull request into the release branch — it is cut from `dev` and bumped in place — but `Gate release` allows one, for a fix going onto a release after the cut. Should that happen, it draws a different set: `Checks`, `Unit tests`, `Gate release` (under the same caveat), and `Version check` in its equality form only. `Changelog entry` fires on a pull request into `main` and nowhere else, so the cut pull request does not check the changelog.

What is still *only* this skill's job: CI cannot tell whether the vendored hook bundle is behind its sources, which is the first gate's question, and no ruleset requires any of the checks above, so a red one refuses no merge. Read them and stop on a failure rather than relying on GitHub to hold the line. `make test-recipes` does not run on this pull request either — `Recipes` is nightly, on demand, and on a pull request touching the recipe sources — which matches the gates above, where it is not a release gate.
...
- **A release does not refresh the vendored hook bundle; it refuses a stale one.** `templates/hooks/assets/check.mjs` is built in `pipelex-sdk-js` and copied in by `make vendor-hook` (from `SDK_JS_DIR`, defaulting to `../pipelex-sdk-js`), and no build step runs it. The first gate is what notices a bundle that is behind, and the re-vendor it asks for reaches `dev` on its own branch before the release is cut, never inside the release commit, which carries the version files and what the build restamps. `make check`, in the second gate and in `Checks`, separately refuses a bundle whose provenance names an unreleased engine or no SDK commit.
