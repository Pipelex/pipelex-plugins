...
**Checkpoint B** — ready for a PR against `dev` with `Closes <item>` in the body. Update this document with what shipped and what was deferred. Version stays at `0.5.0` and the entries stay under `[Unreleased]` until the `/release` skill cuts the next plugin release; the skill's "v0.1.0" is the scope named above, not a number in a manifest.
