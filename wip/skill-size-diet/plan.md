...
## Handoff — 2026-09-23, session db12d401, before a compaction

- **Phase 0** (`L-260923-75fb17`): the facts are in [`facts.md`](facts.md), committed `966b78f` on `feature/Diet-facts-script-paths` (worktree `_pipelex-plugins--diet-facts-script-paths`). The ceiling is 13,000 characters (5,000 tokens at 2.77 characters per token, the lowest Claude 5 ratio measured, less about 6 %). The skill-directory variable is `skill_dir`: `${CLAUDE_SKILL_DIR}` on Claude, a `<skill-dir>` placeholder defined in one sentence on Codex and Vibe, and a script is run by its absolute path from the user's project through its interpreter. Still owed: tick the phase 0 boxes below, then the pull request (`Closes L-260923-75fb17`, `Advances L-260923-a9bdfe`), `/rev`, merge, `/ledger-land`.
- **Side fix** `L-260923-dfb8ee`, found in phase 0: Mistral Vibe drops `pipelex-design` and `pipelex-scaffold`, whose descriptions carry a `: `. Fixed in `efa2c5b` on `feature/Vibe-drops-two-skills` (worktree `_pipelex-plugins--vibe-drops-two-skills`), with a strict-YAML frontmatter check in `make check`. Still owed: `/rev`, pull request, merge, land.
- **Phase 1** (`L-260923-8c296f`): work in progress `45aae80` on `feature/Shared-blocks-diet-machinery` (worktree `_pipelex-plugins--shared-blocks-diet-machinery`) — the scripts copy, the ceiling and link checks, the guard registry, `shared/credentials.md`, the shrunk requirements block, the `runs/` exclusion in the submission include, the project-root include, one stale-types wording, the frontmatter's blank line. Still owed: the `skill_dir` variable, `CLAUDE.md`, `docs/build-targets.md` and `docs/decisions.md`, the phase 1 ticks and checkpoint 1, two ledger items (integrate's call site loads `runs/` `.mthds` files; skills name `/pipelex-x` to Codex users, whose mention is `$pipelex:pipelex-x`), a rebase onto `dev`, `/rev`, pull request, land.
