...
- [x] `/rev`. Three rounds at profile 4 (cubic, Codex review and adversarial, code-review). The first, whose adversarial pass approved, restored the no-signal row's interactive question the rewrite had dropped, made `user-data.md`'s report follow the skill's step 5 rather than a section number it clashed with, gave a list input's copied files distinct names, and brought the campaign records up to date. The second restored the instruction to skip preparation when every file value is already remote, which the classification had deleted as a duplicate of a sentence that only described the case, and closed a gap older than the diet: step 5 now deletes a prepared file an earlier pass left, which a skipped, declined or failed preparation would otherwise leave for `/pipelex-run` to run stale or to hand back. The third, at the necessity bar, spared the Template strategy that deletion, since a template pass may have just replaced the values the prepared file alone still holds, and gave a list input's files a directory of their own after the first round's naming rule let a list item alias another input's copy. What it found older than the diet is deferred below.
