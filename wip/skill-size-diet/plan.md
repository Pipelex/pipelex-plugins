...
- [x] `/rev`. One round at profile 4 (cubic, Codex review and adversarial, code-review); the adversarial pass approved. It restored the no-signal row's interactive question the rewrite had dropped, made `user-data.md`'s report follow the skill's step 5 rather than a section number it clashed with, gave a list input's copied files distinct names, and brought the campaign records up to date.

- **Rendered sizes after the rewrite**: 12,798 characters on Claude, 12,701 on Codex and 12,722 on Vibe after the review's fixes, all under the 13,000 ceiling; the Claude render was 41,480 characters before. That is above the phase's target of about 12,000, and the difference is checkpoint 2's correction applied before the review rather than after it: reading the draft against the classification's quotes restored qualifiers and remedies a shorter draft had dropped — the `mthds_list_methods` fallback, the scope of a mid-run switch to interactive mode, "with no further context" on the no-signal row, "the same prohibition applies after an upload failure", "no `explicit` flag" on the prepare call, the webapp editor and "source the user does not have" on an invalid template, what the one-line prepare report says. What the rest of the render holds is the guards, the sentences the tests pin, and the fixed blocks: the frontmatter (about 990 characters), the `validate-call` include (about 750) and the requirements include (about 560).
