...
**Decision: the skill always writes each method to its own directory, and when `orphans[]` is non-empty it names them, says what they are, never deletes one, and carries on to finish the integration** (amended 2026-09-13: "and does nothing else" was the ambiguity this section left behind, and it was ruled rather than reworded — the run continues and the report carries the whole weight; `docs/decisions.md` → "Orphans on a successful write do not end the run" is the record). The tool never deletes an orphan and neither does the skill: the moment two methods share a directory, "clean up the orphans" deletes real files. A non-empty `orphans[]` on a fresh integration means the chosen directory was not fresh — an earlier generation, a different target, or an engine rename — and the report says a dedicated directory per generation is the fix, in the tool's own words. On a refresh into the method's own directory, an orphan is usually an artifact the engine stopped emitting — but not always, because the ruling's own continuation writes a sidecar claiming a directory another method's tree may still occupy, so a later refresh can meet that tree and overwrite its same-named files again; the report names the orphans, says which of the two it cannot tell apart, and leaves the decision to the user.
