...
| `mthds_get_method` is absent while the other tools answer | explains the id at contract level and says the source was not read; names both causes — the hosted console, which serves no catalog read, and a workshop that predates the tool |
...
| a **catalog id** does not validate | explains it from the source read above with `validation_errors[]` beside it, and says the fix is not on this disk |
| a published **address** does not validate | reports `validation_errors[]` and stops — there is no source to explain and the fix is upstream |
