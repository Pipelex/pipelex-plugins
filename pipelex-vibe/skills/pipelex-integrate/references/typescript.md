...
**Where those pages are, and which address to write down.** The SDK ships its documentation inside the published package, so every page named here is in the project at `node_modules/@pipelex/sdk/docs/<page>.md` — read it there, because that copy is the version the project installed — and the same page is published at `https://github.com/Pipelex/pipelex-sdk-js/blob/main/docs/<page>.md`. A reference a reader outside this session will see — the report, the module's docstring, a comment — carries the URL and never the `node_modules` path, which means nothing to someone reading the project's source.
