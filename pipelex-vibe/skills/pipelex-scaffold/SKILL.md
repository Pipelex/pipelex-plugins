...
- **The project's own instructions and skills load in a session started inside it.** Its `CLAUDE.md` / `AGENTS.md` and its own skills (`bump-sdk` and the like, and a starter's `release`) are not in the current session; `cd <dir>`, then starting Mistral Vibe there, is how they arrive.
