...
`<skill-dir>` stands for the directory holding this `SKILL.md`, the one Mistral Vibe named when it loaded the skill. Run the script, never its steps by hand:
...
- **The project's own instructions and skills load in a session started inside it.** Its `CLAUDE.md` / `AGENTS.md` and its own skills (`bump-sdk` and the like) are not in the current session; `cd <dir>`, then starting Mistral Vibe there, is how they arrive.
