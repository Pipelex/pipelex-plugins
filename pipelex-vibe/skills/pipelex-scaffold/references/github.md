...
**A method app inside another repository's work tree has neither a repository nor a pristine commit of its own**, which its `git:` line says, and `gh` refuses it. Create nothing on GitHub: say that the app is part of the enclosing repository, whose own remote carries it, and never follow `gh`'s hint to `git init` the directory, which would plant a repository inside the user's.
