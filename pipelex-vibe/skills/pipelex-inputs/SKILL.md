...
2. **Branch on the structured verdict.** Unlike `mthds_inputs_template`, this tool has **no produced-invalid arm** — it never returns a `validation_errors[]` list:
   ...
   - `status: "error"`, class `input_domain` → either the closure didn't resolve (invalid bundle, unknown `pipe_ref`, unresolvable `main_pipe`) or an asset was unreadable or rejected. This tool delegates its diagnostics: get the structured errors from `mthds_validate` / `mthds_inputs_template`, repair, then retry.
