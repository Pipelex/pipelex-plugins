...
**On a `files` run the suspicion is sharper than a suspicion.** Per step 5, no `.py` file travels with a `files` submission, so a `PipeFunc` naming a function the plane does not already have registered could not have resolved — say that as the cause rather than offering it as one candidate among several, and give the cure: `/pipelex-catalog` saves the bundle with its Python, and the method run by its id alone gets both.
...
| a `files` run's method holds a custom `PipeFunc` | says the Python did not travel and names `/pipelex-catalog` as the path that carries it; a files run has no channel for it |
