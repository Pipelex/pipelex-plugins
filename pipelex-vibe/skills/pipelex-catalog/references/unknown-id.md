...
Read this when `mthds_save_method` answers an `input_domain` error at `method_id`, before concluding anything about the directory's link. The skill's guard on the link holds throughout: it is removed only on the user's yes.
