...
- **`/pipelex-integrate` still works from here meanwhile**, because the Pipelex workshop writes anywhere under the directory the harness was launched in, and the new project sits there. Hand the method to it with `/pipelex-integrate`; a bundle that lives elsewhere on disk is copied into the project by that skill. No method yet → `/pipelex-design` first.
