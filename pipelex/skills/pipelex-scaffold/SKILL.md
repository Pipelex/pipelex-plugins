...
2. **When `<dir>` held a `.git` before step 1, ask before running the script**, showing `git -C <dir> status --short`, since the commit lands on the user's branch. Run the script, never its steps by hand:
