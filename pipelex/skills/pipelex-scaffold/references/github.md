...
**A project inside another repository's work tree has neither a repository nor a pristine commit of its own**, on either branch: the method app's `git:` line says so, and so does the pristine-commit script's `inside:` verdict. `gh` refuses it. Create nothing on GitHub: say that the project is part of the enclosing repository, whose own remote carries it, and never follow `gh`'s hint to `git init` the directory, which would plant a repository inside the user's.
