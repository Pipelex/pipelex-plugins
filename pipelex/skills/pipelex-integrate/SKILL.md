...
- Success with **`orphans[]` non-empty** → the chosen directory was not fresh (an earlier generation, another target, an engine rename). **Carry on to step 7 and report loudly.** The write succeeded and the check owns the tree, so finish the integration — the sidecar, the dependencies, the call site — and never delete an orphan. The report names the orphan paths, says that a directory already holding another generation's artifacts was written into and that same-named files of another method may therefore have been overwritten by the write that just succeeded — the verdict cannot tell that apart from stale leftovers — and gives a dedicated directory per generation as the fix, in the tool's own words. `orphans_truncated: true` → say orphan detection was partial rather than reporting a clean tree. **The branch below ends the run and this one does not, and the asymmetry is deliberate**: there the check disowns what was written, so there is nothing worth finishing; here the check says the tree is current and the usual causes are benign, so the correct integration ships and the report is the whole protection for a directory the user must still tidy.
...
What was generated and where; the target and why; the call site's signature; what changed in the tooling config; how to refresh (this skill again after a bundle edit); any orphans step 6 reported — their paths, that same-named files of another method may have been overwritten by the write that succeeded, and a dedicated directory per generation as the fix — restated here rather than left behind at step 6, because this report is the whole protection the user gets; for a `python-pydantic` project, that no offline drift check exists yet and refresh is the guard; for a `method_id` source, that the catalog is unversioned. Then the hand-off: `/pipelex-inputs` prepares inputs and offers a run.
...
One `mthds_codegen` call with the recorded arguments. Read what happened from the lock's **`artifacts[].content_hash`**, not from the fingerprint: `crate_fingerprint` covers the whole bundle, so a prompt-only edit moves it — and with it the one stamp line at the head of every artifact — while the projected code is byte-identical. Content hashes unchanged therefore means a pure restamp: say so, and leave that one-line-per-file diff to the user's commit. A content hash that moved is the concept set actually moving. **A non-empty `orphans[]` does not end a refresh either**, exactly as in step 6: the regeneration is the engine's own write and the check owns what it produced, so the refresh runs on through the type checker, the call site and the sidecar, and the report names the orphan paths, never deletes them, says the directory holds artifacts this generation does not list — an earlier engine's file names, a target switch, or another method's tree in a directory that was never dedicated — and gives a dedicated directory per generation as the fix. On that last cause the regeneration has overwritten the other method's same-named files again, so the report says so rather than describing the orphans as merely stale — the verdict cannot tell the two apart here any more than in step 6. `orphans_truncated: true` → say detection was partial. An `is_current: false` still ends the refresh, there as here. Then the type checker. **The call site is edited only if it no longer type-checks or the `pipe` record no longer matches the signature** — a renamed input, a reshaped output — and the edit is the minimal one, stated in the report. The sidecar is rewritten last. A directory holding a `codegen.lock` with no sidecar naming this method is not this method's (step 4): choose another directory name and say why, or let the user overrule by naming the method that directory is for, as step 4 allows. The exception is a harness-owned layout, which keeps no sidecar by design — see that section.
...
| success with `orphans[]` non-empty | **continue to step 7** — report by name, never delete; say another generation's artifacts were in the directory and that this write may have overwritten same-named files of another method; a dedicated directory per generation is the fix. `orphans_truncated: true` → say detection was partial. Unlike the row below, this one does not end the run: the check owns this tree (step 6) |
