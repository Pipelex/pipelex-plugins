...
Run it from the project root with **the project's own environment**, the one step 8 put `pipelex-sdk` in: `uv run python scripts/codegen_check.py …` under uv, `poetry run python …` under poetry, `pipenv run python …` under pipenv, and the active environment's `python` otherwise. An interpreter without the SDK does not guess: it exits `2` and names the fix. A check that raises instead of reaching a verdict — a lock `tomllib` refuses, a directory the process cannot search — is `2` too, for that directory, with the error on stderr, and the directories after it are still checked. That is the one place the script catches `Exception`, so a project whose Ruff selects `BLE001` gets a per-file ignore for `scripts/codegen_check.py` rather than an edit to the script. Extend the project's existing gate rather than inventing one — for a Makefile, a target the `check` target depends on:
