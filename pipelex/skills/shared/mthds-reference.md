...
Output is `Page[]` (a list of pages with `text_and_images`, plus a `page_view` image of each page only when the pipe sets `page_views = true`). The other optional fields are `page_views_dpi`, `max_page_images` (`0` keeps no embedded images), `page_image_captions`, and, for web pages only, `render_js` and `include_raw_html`.
...
**Structured content is auto-expanded**: When you use `@structured_var`, all fields of the structured concept are automatically formatted. No need to manually enumerate fields.
