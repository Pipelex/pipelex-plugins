...
- **If the tool is absent from this session** (the MCP server isn't connected), STOP and tell the user in one line: *"The Pipelex MCP server isn't connected — check the plugin's MCP connection (`/mcp`); the plugin manifest must point at a running `pipelex-mcp` server."* Do not touch the bundle files without validation available.
