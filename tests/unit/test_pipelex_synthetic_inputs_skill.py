...
class TestPipelexSyntheticInputsSkillShape:
    ...
    def test_the_refusal_is_the_skills_alone_and_sees_a_scanned_photo(self) -> None:
        """The refusal of photographs and handwriting is a guard, so it lives in `SKILL.md` and not in the PNG
        recipes, which are read at step 4, after step 1 has decided it. The one exception it carries — a photo of
        a receipt is a `document_scan` — sat in `png.md`, where a model refusing at step 1 never read it."""
        for target_name in TARGETS:
            body = self.render(target_name)
            refusal = self.the_line(body, "**Not covered, by design:** photographs and handwriting.")
            assert "A photo of a receipt or a form is a `document_scan`, which this skill makes" in refusal
