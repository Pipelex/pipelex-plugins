...
- **Claude Code.** Set `command = "node"` and `args = ["/path/to/pipelex-mcp/dist/local/main.js"]` in that block, run `make build`, and reload as above. Switch it back before you commit, since the build writes the block into every target.
