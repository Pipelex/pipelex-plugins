...
| `validate-call.md.j2` | how a local bundle is handed to the workshop: which files (every `.mthds` file beneath the bundle directory except `runs/`), the path form of `files` with the workshop's refusal of a path outside its working directory and the relaunch that cures it, and the inline fallback the hosted console needs | `validate_call_file_set` (`false` in `pipelex-integrate` alone, which states the file set its call site loads); `validate_call_inline` (what is said of the inline form after its shape, the hosted-console fallback by default; set by `pipelex-catalog` alone, since a save sent inline is left unlinked) |
