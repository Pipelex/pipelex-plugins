...
- **Pinned manifest shapes** (verified live on both platforms, `mthds_validate` round-trips with a real key): Claude `{"type": "stdio", "command": "npx", "args": ["-y", "@pipelex/mcp@latest"]}` — the spawned server inherits the full shell env, so `PIPELEX_API_KEY` just works. *(Superseded for credentials by the userConfig decision below: the Claude entry now spawns the `launch-pipelex-mcp.sh` wrapper with user-config values injected; the direct shape remains the fallback when no `user_config` block is configured.)* Codex: same minus `type` (transport picked structurally), **plus `"env_vars": ["PIPELEX_API_KEY", "PIPELEX_BASE_URL"]`** — Codex spawns MCP servers with a minimal whitelist env (shell env is not passed through; `shell_environment_policy` doesn't apply to MCP spawns), and `env_vars` forwards variables *by name* from the user's own env. `PIPELEX_BASE_URL` is forwarded too because a key is only valid against its own base URL (a dev key against the default prod URL gets `Forbidden`).
...
- **The MCP entry spawns a wrapper, not the workshop directly.** `${user_config.*}` values are injected as `PIPELEX_PLUGIN_<KEY>` env vars (naming convention: option `foo` → `PIPELEX_PLUGIN_FOO` → promoted to `PIPELEX_FOO`), and `hooks/launch-pipelex-mcp.sh` promotes each to its real `PIPELEX_*` name **only when non-empty** before exec'ing the workshop command. Rationale for the indirection: the substitution semantics for an *unset* option are undocumented, and setting `PIPELEX_API_KEY=""` directly in the entry's `env` could clobber a working key inherited from the shell on terminal launches. The wrapper makes precedence explicit: set user-config wins, session env is the fallback, keyless stays fail-open.
