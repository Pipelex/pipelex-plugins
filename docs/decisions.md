...
The plugin declares the **`pipelex-mcp`** server (streamable HTTP; tools `mthds_validate`, `mthds_inputs_template`) so the MCP-backed skills (`pipelex-design`, `pipelex-organize`, `pipelex-inputs`) can call it natively — no vendored script, no `curl` recipes.
...
- **The baked default is currently the `pipelex-mcp` Alpic dev tunnel** — an interim so marketplace installs in env-var-less harnesses (the Claude desktop app) reach a live server; switch it to the stable URL when `pipelex-mcp` deploys. Use the `PIPELEX_MCP_URL` override for a different server (local dev: `http://localhost:3000/mcp` via `make dev` in `../pipelex-mcp`).
