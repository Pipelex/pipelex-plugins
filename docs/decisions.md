...
- **Pinned manifest shapes** (verified live on both platforms, `mthds_validate` round-trips with a real key): Claude `{"type": "stdio", "command": "npx", "args": ["-y", "@pipelex/mcp@latest"]}` — the spawned server inherits the full shell env, so `PIPELEX_API_KEY` just works. Codex: same minus `type` (transport picked structurally), **plus `"env_vars": ["PIPELEX_API_KEY", "PIPELEX_BASE_URL"]`** — Codex spawns MCP servers with a minimal whitelist env (shell env is not passed through; `shell_environment_policy` doesn't apply to MCP spawns), and `env_vars` forwards variables *by name* from the user's own env. `PIPELEX_BASE_URL` is forwarded too because a key is only valid against its own base URL (a dev key against the default prod URL gets `Forbidden`).
...
- **Claude Desktop** is out of scope for the baked launcher (GUI apps inherit no shell env; node may be absent): Desktop users are pointed at the hosted-console connector in the README. The exact Desktop failure UX (silent skip vs error toast) is still unverified — tracked in `TODOS.md` (V-P3).
