...
**`pipelex-inputs`** prepares a method's `inputs.json` from the input template the Pipelex tools derive: a placeholder template, synthetic data, your own files, or a mix. It works for a local bundle, a saved method's catalog id or a published method's `github.com/…` address. It keeps your own files out of git with a `.gitignore` entry, uploads local files to Pipelex storage and writes the run-ready values to `inputs.prepared.json` beside the source `inputs.json`, which it never rewrites, and it closes by offering the run, even when you said not to run, since the offer starts nothing until you say yes. A skill that hands it the work can ask it to stop at run-ready with no offer, as `pipelex-lab` does for a test case whose answer key is not written yet.
...
**`pipelex-integrate`** wires a method into an existing TypeScript or Python codebase. The Pipelex tools write generated, drift-proof types into one directory per method; the skill excludes that tree from your formatters, records a `sources.json` sidecar, adds the offline drift check to your project's gate, and writes one typed call site that runs the method through `@pipelex/sdk` or `pipelex-sdk`, naming its pipe by the qualified reference the validation returns, `domain.pipe_code`. A project made from a Pipelex template keeps its own codegen harness.

**`pipelex-scaffold`** starts a project where none exists. A TypeScript web app around a method comes from the method-app template, created for the method by the template family's own initializer and left running by the template's `make serve`, with its URL reported. Every other project, a Python CLI or service included, comes from its language's own initializer, is committed and given its env file by the skill's own scripts, and is handed to `/pipelex-integrate`. On either branch, a project created inside another repository's work tree gets no repository and no commit of its own: its files are new files of that repository, for you to review and commit there. On the initializer branch, when that repository ignores the project's directory, nothing versions the project, and the skill says so and leaves making it a repository to you.
