...
├── skills/
│   ├── pipelex-explain/SKILL.md.j2   # Read-and-explain a bundle directory; strictly read-only (workshop optional)
...
- `floors` — a table (`[vars.floors]`) of the minimum versions the skills state to the user: `pipelex_sdk_js`, `pipelex_sdk_py`, `pipelex_mcp`, `node`, `python_min`, `python_max`, `method_app_port`. `pipelex-integrate` and `pipelex-scaffold` read them as `{{ floors.<key> }}`, so a bump is one edit. **`pipelex_mcp` is a ceiling, not a floor** — it names the last workshop release that *predates* the main-pipe signature, so bumping it means re-reading the sentence rather than renumbering it. The static references under `skills/` are copied verbatim and keep their literals; `scripts/check.py`'s `check_version_floors` holds them to the table and, separately, catches a misspelled key, which Jinja would otherwise render as the empty string with every gate green. Bump the table, `make build`, `make check`, and the check names what still says the old number.
