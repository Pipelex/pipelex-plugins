...
│   ├── pipelex-scaffold/SKILL.md.j2  # Front door to a new project: the method-app template through its own `make create`, left running; or the ecosystem's initializer, committed and given its env file by the skill's scripts (no MCP dependency)
...
**A block several skills say word for word lives in one include.** The MCP requirements' two stops, the submission convention (the file set and the `files` path form), the project-root markers, the stale-types notice, the saved-copy notice, the catalog-id pointer and bridge and the PipeFunc warning are include-only partials under `templates/skills/shared/`; a skill sets what it words differently with `{% set %}` and includes the rest. Never paste one of those blocks into a new skill — `tests/unit/test_gen_skill_docs.py::TestSharedSkillIncludes` fails when a block has more than one source. The partials, their parameters and the two whitespace mechanics are in `docs/build-targets.md`.
