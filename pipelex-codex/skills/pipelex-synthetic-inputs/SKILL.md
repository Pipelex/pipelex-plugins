...
- **Deterministic where the format allows it.** Seed every source of randomness (`SEED` in each content block) so a regenerated file draws the same pixels, and pass `invariant=1` to reportlab so a PDF is byte-identical across runs. Word and Excel files embed a creation timestamp that neither library lets you fix, so two `docx`/`xlsx` renders of the same content differ byte for byte — the content is reproducible, the bytes are not.
