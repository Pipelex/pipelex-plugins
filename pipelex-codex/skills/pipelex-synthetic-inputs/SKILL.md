...
**Rung 2 — no `uv`, but `python3` with `venv` and `pip`.** When the preflight prints nothing, read [venv.md](references/venv.md) before creating anything: it builds the venv this skill owns under `${XDG_CACHE_HOME:-$HOME/.cache}/pipelex-plugins/synth-venv` and gives the runner line.
