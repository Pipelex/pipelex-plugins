...
**Check containment before writing anything**: the project root must sit under the directory this session was launched in, both resolved (`pwd -P`, `realpath`), or STOP: relaunch the harness from the project root. **Never write the tree into the workshop's directory and move it across.**
...
`<skill-dir>` stands for the directory holding this `SKILL.md`, the one Codex named when it loaded the skill. Copy the gate from the project root with `cp`, never by rewriting it, and never format the copy:
