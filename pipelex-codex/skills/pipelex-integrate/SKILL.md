...
- **If a tool is absent from this session**, the Pipelex MCP server isn't connected: STOP, and tell the user in one line what [the connection reference](../shared/credentials.md#the-tool-is-absent) says for Codex.
...
**Check containment before writing anything**: the project root must sit at or under the directory this session was launched in, both resolved (`pwd -P`, `realpath`), or STOP: relaunch the harness from the project root. **Never write the tree into the workshop's directory and move it across.**
...
`method` holds the selector as passed; paths are from the project root; `pipe` writes `Concept[]` for a list, `Concept?` for an optional. **`bundle_dir` is the call site's `BUNDLE_DIR`**, and `sources` hashes the raw bytes of every `.mthds` file beneath it with SHA-256; a `method_ref` or `method_id` has no `bundle_dir` and an empty `sources`.
...
`<skill-dir>` stands for the directory holding this `SKILL.md`, the one Codex named when it loaded the skill. Copy the gate from the project root with `cp`, never by rewriting it, and never format the copy. **A file already there without the line `Copied verbatim into a project by /pipelex-integrate` is the user's: ask first.**
...
| Condition | Do this |
|---|---|
| invalid, unrunnable, or `pending_signatures` | STOP, nothing generated: `validation_errors[]` to `/pipelex-design` or `/pipelex-edit`, else finish it in `/pipelex-design` |
...
| a gate exits `2` | no verdict, not drift: read [references/gate-failures.md](references/gate-failures.md) |
| `stale-source`: a file added, or no `bundle_dir` | [refresh mode](references/refresh.md); never add its hash to `sources` by hand |
