...
2. **Branch on the structured verdict.** Unlike `mthds_inputs_template`, this tool has **no produced-invalid arm** — it never returns a `validation_errors[]` list:
   ...
   - `status: "error"`, class `config` → stop per the Requirements above and surface the `hint` verbatim. **One `config` refusal is not about the credential**, and it reaches only a by-address target: this tool resolves an address's signature through the run route, which refuses a published package that ships in-process Python where the deployment does not host a sandbox. When `mthds_inputs_template` has just answered on the very same address, the key is not what failed — say the package cannot be prepared on this deployment, and do not send the user to their credentials.
