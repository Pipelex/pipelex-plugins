...
Read this when an MCP-backed skill has stopped: because a Pipelex MCP tool is absent from the session, or because a call returned a `config`-class error about the API key. It says what to tell the user on Codex; the skill's own stop still holds, so answer from here and do not carry on.
